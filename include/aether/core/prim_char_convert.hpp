...
AETHER_INLINE void prim_to_charT<1>(const CellsView &prim, CharView &chars, const eigenvec_view &eigs){
    ...
    #pragma omp parallel for schedule(static) default(none) shared(prim,chars,eigs,N)
...
template <>
AETHER_INLINE void char_to_primT<1>(const CharView &chars, CellsView &prim, const eigenvec_view &eigs){
    std::size_t N = prim.ext.flat();
    constexpr int numvar = aether::phys_ct::numvar;
