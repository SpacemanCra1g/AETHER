...
inline constexpr int dim = AETHER_DIM;
  ...
  // If Extra transverse velocities are tracked, we add that in here
  #if AETHER_DIM_FORCING
      #if AETHER_DIM == 1
        #if AETHER_EXTRA_DIM == 2
          inline constexpr int numvar = 3 + dim + 1;
        #elif AETHER_EXTRA_DIM == 3
          inline constexpr int numvar = 3 + dim + 2;
        #endif
      #elif AETHER_DIM == 2
        #if AETHER_EXTRA_DIM == 2
          inline constexpr int numvar = 3 + dim;
        #elif AETHER_EXTRA_DIM == 3
          inline constexpr int numvar = 3 + dim + 1;
        #endif
      #endif
  ...
  inline constexpr int numvar = 6;
#elif AETHER_PHYSICS_KIND == 3 // (M)Hydrodynamics
  inline constexpr int numvar = 3 + 2 * dim;
#else
