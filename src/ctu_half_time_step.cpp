...
// Reference for this file 'An Unsplit Godunov Method for Ideal MHD via
// Constrained Transport by Thomas A. Gardiner
...
KOKKOS_INLINE_FUNCTION prims load_face_prims(const FaceT &faces,
                                             idx index) noexcept {
    prims x{};
...
template <class FaceT>
KOKKOS_INLINE_FUNCTION void store_face_prims(const FaceT &faces, idx index,
    ...
    if constexpr (P::HAS_VZ)
        faces(P::VZ, index.q, index.k, index.j, index.i) = x.vz;
    faces(P::P, index.q, index.k, index.j, index.i) = x.p;
}

KOKKOS_INLINE_FUNCTION
bool prims_valid(const prims &x) noexcept {
    return std::isfinite(x.rho) && std::isfinite(x.p) && (x.rho > 0.0) &&
           (x.p > 0.0);
}

template <class FaceT>
KOKKOS_INLINE_FUNCTION void backup_cons(FaceT &backup, idx index,
    ...
    if constexpr (C::HAS_MZ)
        backup(C::MZ, index.q, index.k, index.j, index.i) = x.mz;
    backup(C::E, index.q, index.k, index.j, index.i) = x.E;
}

template <class FaceT>
KOKKOS_INLINE_FUNCTION cons load_backup_cons(FaceT &backup,
                                             idx index) noexcept {
    cons x{};
    ...
    x.mz =
        (C::HAS_MZ) ? backup(C::MZ, index.q, index.k, index.j, index.i) : 0.0;
...
KOKKOS_INLINE_FUNCTION cons add_flux_diff(const cons &base, const FluxT &F,
                                          idx idx_lo, idx idx_hi,
                                          const double coeff) noexcept {
    cons x = base;
    x.rho += coeff * (F(C::RHO, idx_hi.q, idx_hi.k, idx_hi.j, idx_hi.i) -
                      F(C::RHO, idx_lo.q, idx_lo.k, idx_lo.j, idx_lo.i));
    x.mx += coeff * (F(C::MX, idx_hi.q, idx_hi.k, idx_hi.j, idx_hi.i) -
                     F(C::MX, idx_lo.q, idx_lo.k, idx_lo.j, idx_lo.i));
    x.my += coeff * (F(C::MY, idx_hi.q, idx_hi.k, idx_hi.j, idx_hi.i) -
                     F(C::MY, idx_lo.q, idx_lo.k, idx_lo.j, idx_lo.i));
    if constexpr (C::HAS_MZ) {
        x.mz += coeff * (F(C::MZ, idx_hi.q, idx_hi.k, idx_hi.j, idx_hi.i) -
                         F(C::MZ, idx_lo.q, idx_lo.k, idx_lo.j, idx_lo.i));
    }
    x.E += coeff * (F(C::E, idx_hi.q, idx_hi.k, idx_hi.j, idx_hi.i) -
                    F(C::E, idx_lo.q, idx_lo.k, idx_lo.j, idx_lo.i));
    return x;
}
...
KOKKOS_INLINE_FUNCTION
prims recover_prims_or_fallback(cons corrected, cons fallback,
...
correct_face_2d(FaceOutT &out_faces, idx idx_o, FluxT &transverse_flux,
                idx idx_lo, idx idx_hi, double coeff, double gamma) noexcept {
...
KOKKOS_INLINE_FUNCTION void
write_half_step_with_fallback(FaceOutT &out_faces, idx idx_o, BackupT &backup,
                              Flux1T &F1, idx f1_lo, idx f1_hi, const double c1,
                              Flux2T &F2, idx f2_lo, idx f2_hi, const double c2,
...
void ctu_half_time_correction<2>(Simulation &sim, Simulation::View view) {
    ...
    Kokkos::parallel_for(
        ...
        Kokkos::MDRangePolicy<exec_space, Kokkos::Rank<2>>({jb - 1, ib - 1},
                                                           {je + 1, ie + 1}),
        KOKKOS_LAMBDA(const int j, const int i) {
            for (int q = 0; q < quad; ++q) {
                ...
                detail::correct_face_2d(Lx, x_right, Fy, y_bottom, y_top,
                                        -dyt_half, gamma);
                detail::correct_face_2d(Rx, x_left, Fy, y_bottom, y_top,
                                        -dyt_half, gamma);
                detail::correct_face_2d(Ly, y_top, Fx, x_left, x_right,
                                        -dxt_half, gamma);
                detail::correct_face_2d(Ry, y_bottom, Fx, x_left, x_right,
                                        -dxt_half, gamma);
            }
        });
}
            ...
            for (int q = 0; q < quad; ++q) {
...

                {
                    ...
                    detail::store_face_prims(
                        ...
                        detail::recover_prims(
                            detail::add_flux_diff(q0, Fy, y_bottom, y_top,
                                                  -dyt_third),
                            gamma));
                    ...
                    detail::store_face_prims(
                        ctu_Lx, x_right,
                        detail::recover_prims(
                            detail::add_flux_diff(q0, Fz, z_down, z_up,
                                                  -dzt_third),
                            gamma));
                }
...

                {
                    ...
                    detail::store_face_prims(
                        Ly, y_top,
                        detail::recover_prims(
                            detail::add_flux_diff(q0, Fx, x_left, x_right,
                                                  -dxt_third),
                            gamma));
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            for (int q = 0; q < quad; q++) {
                ...
                detail::write_half_step_with_fallback(
                    Lx, x_right, xL_bak, ctu_Fy, y_top, y_bottom, +dyt_half,
                    ctu_Fz, z_up, z_down, +dzt_half, gamma);

                detail::write_half_step_with_fallback(
                    Rx, x_left, xR_bak, ctu_Fy, y_top, y_bottom, +dyt_half,
                    ctu_Fz, z_up, z_down, +dzt_half, gamma);

                detail::write_half_step_with_fallback(
                    Ly, y_top, yL_bak, ctu_Fx, x_right, x_left, +dxt_half, Fz,
                    z_up, z_down, +dzt_half, gamma);

                detail::write_half_step_with_fallback(
                    Ry, y_bottom, yR_bak, ctu_Fx, x_right, x_left, +dxt_half,
                    Fz, z_up, z_down, +dzt_half, gamma);

                detail::write_half_step_with_fallback(
                    Lz, z_up, zL_bak, Fx, x_right, x_left, +dxt_half, Fy, y_top,
                    y_bottom, +dyt_half, gamma);

                detail::write_half_step_with_fallback(
                    Rz, z_down, zR_bak, Fx, x_right, x_left, +dxt_half, Fy,
                    y_top, y_bottom, +dyt_half, gamma);
            }
        });
}
#endif
